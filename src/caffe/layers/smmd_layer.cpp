...
void SMMDLossLayer<Dtype>::LayerSetUp(
  ...
  //num_class_ = bottom[2]->count(0);

  count_soft = new Dtype[num_class_];
  avg_entropy = new Dtype[num_class_];
  count_tmp = new int [num_class_];
  caffe_set(num_class_,Dtype(0.0),count_soft);
  caffe_set(num_class_,Dtype(0.0),avg_entropy);
  count_hard = new int[num_class_];
  caffe_set(num_class_,0,count_hard);
  caffe_set (num_class_,0,count_tmp);
  source_num_batch = new int[num_class_];
  target_num_batch = new int[num_class_];
  source_num_resamp = new int[num_class_];
  target_num_resamp= new int[num_class_];
  caffe_set(num_class_,0,source_num_batch);
  caffe_set(num_class_,0,target_num_batch);
  caffe_set(num_class_,0,source_num_resamp);
  caffe_set(num_class_,0,target_num_resamp);
  cross_entropy = 0.0;
...
template <typename Dtype>
void SMMDLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  NeuronLayer<Dtype>::Reshape(bottom, top);
}

template <typename Dtype>
void SMMDLossLayer<Dtype>::Forward_cpu(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
}

template <typename Dtype>
void SMMDLossLayer<Dtype>::Backward_cpu(
  const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
  const vector<Blob<Dtype>*>& bottom) {
}

#ifdef CPU_ONLY
STUB_GPU(SMMDLossLayer);
#endif

INSTANTIATE_CLASS(SMMDLossLayer);
REGISTER_LAYER_CLASS(SMMDLoss);
