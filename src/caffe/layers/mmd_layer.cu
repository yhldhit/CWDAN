...
void MMDLossLayer<Dtype>::Backward_gpu(
    ...
    int *src_class_distr = new int [class_num];
    int source_sam;
    //const int class_num = bottom[2]->count(0);
    if (src_domain_ == 0){
        /*******for amazon********/
        int amazon_10[10] = {92,82,94,99,100,100,99,100,94,98};
        for (int i=0; i<class_num;i++){
          src_class_distr[i] = amazon_10[i];
        }
        source_sam = 958;
    }
    else if (src_domain_ == 1){
        /*******for webcam*******************/
        int webcam_10[10] = {29,21,31,27,27,30,43,30,27,30};
        for (int i=0; i<class_num;i++){
          src_class_distr[i] = webcam_10[i];
        }
        source_sam = 295;
    }
    else if (src_domain_ == 2){
        /*******for dslr*******************/
        int dslr_10[10] = {12,21,12,13,10,24,22,12,8,23};
        for (int i=0; i<class_num;i++){
          src_class_distr[i] = dslr_10[i];
        }
        source_sam = 157;
    }
    else if (src_domain_ == 3){
      /*******for caltech*******************/
      int caltech_10[10] = {151,110,100,138,85,128,133,94,87,97};
      source_sam = 1123;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = caltech_10[i];
      }
    }
    else if(src_domain_ == 4){
      /***********for caltech12***************/
      int caltech_12[12] = {800,110,98,100,101,98,116,102,270,133,798,209};
      source_sam = 2935;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = caltech_12[i];
      }
    }
    else if(src_domain_ == 5){
      /********for voc12**********/
      int voc12[12] = {670,552,765,508,706,421,1161,1286,482,575,526,4087};
      source_sam = 11739;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = voc12[i];
      }
    }
    else if(src_domain_ == 6){
      /********for bing**********/
      int bing[12] = {446,480,446,484,467,518,511,496,463,445,471,489};
      source_sam = 5716;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = bing[i];
      }
    }
    else if (src_domain_ == 7){
      /***********for amazon***************/
      int amazon_31[31] = {92,82,72,82,36,94,91,97,97,81,99,100,100,98,100,99,100,94,96,95,93,100,98,98,90,75,100,99,99,96,64};
      source_sam = 2817;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = amazon_31[i];
      }
    }
    
    else if(src_domain_ == 8){
    //    /***********for webcam*************/
      int webcam_31[31] = {29,21,28,12,16,31,40,18,21,19,27,27,30,19,30,43,30,27,28,32,16,20,30,27,40,11,25,30,24,23,21};
      source_sam = 795;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = webcam_31[i];
      }
    }
    else if(src_domain_ == 9){
      /***********for dslr***************/
      int dslr_31[31] = {12,21,24,12,16,12,13,14,15,15,13,10,24,16,31,22,12,8,10,10,13,15,23,18,10,7,18,26,21,22,15};
      source_sam = 498;
      for (int i=0; i<class_num;i++){
        src_class_distr[i] = dslr_31[i];
      }
    }
    else
      return;
    ...
    for (int i= 0;i<class_num;i++){
        ratio_source[i] = src_class_distr[i]*1.0/source_sam;
    }
    ...
    if (mmd_lock_ == 1 ){
        ...
        if(data_dim_ == class_num){
            ...
            if (total_target_num  >= 1000){//for layer fc8
                ...
                for (int k = 0; k<class_num; k++){
                    ...
                    LOG(INFO)<<"i:"<<k<<"\tweight_tmp:"<<weight_tmp[k]<<"\tweight_sr:"<<ratio_source[k];
        ...
        //change the lock
        if (total_target_num  >= 1000){//for layer fc7 and fc8
            now_iter_ = 0;
            total_target_num = 0;
            all_sample_num_ = 0;
            mmd_lock_ = 1 - mmd_lock_;
